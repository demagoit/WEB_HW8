...
params = {
    ...
    'queue': config.get('RABBIT', 'E_QUEUE')
}
...
def send_email_callback(ch, method, properties, body):
    ...
    print(f" [x] Sending mail to {responce.fullname}...", end='')
    time.sleep(1)
    responce.update(msg_sent=True)
    ch.basic_ack(delivery_tag=method.delivery_tag)
    print(f"mail sent to {responce.e_mail}")
...
def main():
    ...
    channel.basic_consume(queue=params['queue'], on_message_callback=send_email_callback, auto_ack=False)
